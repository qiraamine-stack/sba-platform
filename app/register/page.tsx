...
export default function RegisterPage() {
  ...
  const submit = async () => {
    setLoading(true);
    setError("");
    const res = await fetch("https://web-production-19eab.up.railway.app/api/auth/register", {
      method: "POST",
  ...
  return (
    <main className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
      <div className="bg-white rounded-xl shadow p-8 w-full max-w-md">
        ...
        <p className="text-gray-500 mb-6">Join SBA Platform to buy or sell businesses</p>
        ...
        <div className="mb-6">
          ...
          <select name="role" value={form.role} onChange={handle}
            className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500">
            <option value="buyer">Buyer — looking to acquire a business</option>
            <option value="seller">Seller — listing a business for sale</option>
          </select>
        </div>
